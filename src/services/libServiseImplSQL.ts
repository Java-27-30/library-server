...
export class LibServiceImplSQL implements  LibService{
    ...
    async getAllBooks(): Promise<Book[]> {
        const [result] = await configuration.pool.query('SELECT * FROM books')
        return result as unknown as Book[];
    }

    async getBooksByGenre(genre: BookGenres): Promise<Book[]> {
        const [result] = await configuration.pool.query('SELECT * from books WHERE genre = ?', [genre])
    ...
    async pickUpBook(id: string, reader: Reader): Promise<void> {
        ...
        if (!queriedReader) {
            await configuration.pool.query('INSERT INTO readers VALUES (null, ?)', [reader]);
            queriedReader = await this.getReaderByName(reader.userName);
        }

        await configuration.pool.query('INSERT INTO books_readers VALUES(?, ?, ?, ?)',
            [book.id, queriedReader.reader_id, new Date().toDateString(), null]);
        configuration.pool.query('UPDATE books SET status = "on_hand" WHERE id = ?', [id])

    }

    private getReaderByName = async (reader: string) => {
        const query = 'SELECT * FROM readers WHERE name = ?'
        const [result] = await configuration.pool.query(query, [reader]);
    ...
    async removeBook(id: string): Promise<Book> {
        ...
        configuration.pool.query('DELETE FROM books_readers WHERE book_id = ?', [id])
        configuration.pool.query('DELETE FROM books WHERE id = ?', [id])
        return book;
    }
    ...
    async returnBook(id: string): Promise<void> {
        ...
        if (book.status !== BookStatus.ON_HAND)
            throw new HttpError(400,"Wrong book status");
        configuration.pool.query('UPDATE books SET status = "on_stock" WHERE id = ?', [id])
        await configuration.pool.query('UPDATE books_readers SET return_date = ? WHERE book_id = ? AND return_date IS NULL', [new Date().toDateString(), id]);
    }

    async getBookById(id: string): Promise<Book> {
        const query = 'SELECT * FROM books WHERE id = ?';
        const value = [id]
        const [result] = await configuration.pool.query(query, value);
    ...
    getPickRecordsByBookId = async (id: string) => {
        ...
        const [result] = await configuration.pool.query(query, [id])
